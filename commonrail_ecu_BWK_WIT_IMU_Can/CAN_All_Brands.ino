...
if (ISO_Bus.read(msgim)) 
   {
      ...
      j1939_decode(msgim.id, &PGN, &priority, &srcaddr, &destaddr);

      //**Work Message**
      if (PGN == 65093) //Rear hitch data
      {
        ISORearHitch = (msgim.buf[0]); 
      ...
      if (Brand == 3)
      {
          if (msgim.id == 0x18EF2CF0)   //**Fendt Engage Message**  
          {
            if ((msgim.buf[0])== 0x0F && (msgim.buf[1])== 0x60 && (msgim.buf[2])== 0x01){   
              engageCAN = 1;
              relayTime = ((millis() + 1000));
            }
          }
        ...
        if (ShowCANData == 1){
            ...
            Serial.print(", MB: "); Serial.print(msgim.mb);
            Serial.print(", ID: 0x"); Serial.print(msgim.id, HEX );
            ...
            Serial.print(", EXT: "); Serial.print(msgim.flags.extended );
            Serial.print(", LEN: "); Serial.print(msgim.len);
            Serial.print(", DATA: ");
            for ( uint8_t i = 0; i < 8; i++ ) 
            {
              Serial.print(msgim.buf[i]); Serial.print(", ");
            }
